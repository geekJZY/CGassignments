...
Vec3d colorDet::jitter(Vec3d n, float range){
    n = n/norm(n);
...
Vec4f colorDet::colorReturn(int objFrom, int objInx, Vec3d ph, Vec3d nep, int tierCnt){
    ...
    if(objInx >= 0 && objInx < planes.size()){
        ...
        if(planes[objInx].texture == TX_TOTAL_RL){
            ...
            return b*this -> kb*this -> ColorH3 + 0.4* this -> colorReturn(objInx, rayRR.first, rayRR.second, nr, tierCnt + 1) + 0.4* this -> colorReturn(objInx, rayTracing.first, rayTracing.second, nr, tierCnt + 1);
        }
...
//            cout << normVec << endl;
            ...
            return 1*colorSum4f + 0.4* this ->colorReturn(objInx, rayTracing.first, rayTracing.second, nr, tierCnt + 1);
        }
    }
...
//            cout << delta << endl;
            ...
            rayRR = this -> spaceTracer(vRR, ph);
//            cout << "RR" << endl;
//            cout << "nRR is " << nRR << endl;
//            cout << "vRR is " << vRR << endl;
