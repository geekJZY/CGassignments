...
int main(int argc, char *argv[]){
    ...
    Mat img(Size(1280,720),CV_8UC3,color[0]);
    ...
    Mat projectingMap = imread("projImg.jpg");
...
//    cubes.push_back(Cube(points, Vec4f(53,36,155,255),Vec4f(113,86,255,255),Vec4f(1930,1660,2550,2550),Vec4f(0,0,0,255), TX_HALF_RL_HALF_RR, 1.33, quad, sphereTexture2, normalMapCube));
    ...
    spheres.push_back(Quadrics(1,1,1,0,-1,3,3,3,Vec3d(20,8,3),TX_OPAQUE, 1.33,sphereTexture,Vec3d(0,-1,0),Vec3d(-1,0,0),Vec3d(24,2,7),Vec3d(20,-4,4),8,8,Vec4f(0,0,0,255),Quadrics::SOLID_TEXTURE));
    ...
    projecting projector(Vec3d(0.4243,0.4243, 0.8), Vec3d(30, 10, 20), Vec3d(-0.88344,0,0.46855), Vec3d(-0.19880577,0.90555777,-0.37484359), Vec3d(30, 10, 24), 4, 2.5, projectingMap);
        ...
        for(int n = 0; n < N; n ++)
        {
            ...
            for(int xCnt = 0; xCnt < xSample; xCnt ++)
                for(int yCnt = 0; yCnt < ySample; yCnt ++)
                {
                    ...
                    temp += 4*projector.colorCal(rayTracing.second, colorDeter);
