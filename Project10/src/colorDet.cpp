...
Vec4f colorDet::colorReturn(int objFrom, int objInx, Vec3d ph, Vec3d nep, int tierCnt){
    ...
    if(objInx >= 0 && objInx < planes.size()){
        ...
        if(planes[objInx].texture == TX_TOTAL_RL){
            ...
            return b*this -> kb*this -> ColorH3 + 0.7* this -> colorReturn(objInx, rayRR.first, rayRR.second, nr, tierCnt + 1) + 0.1* this -> colorReturn(objInx, rayTracing.first, rayTracing.second, nr, tierCnt + 1);
        }
