...
int main(int argc, char *argv[]){
    ...
    Mat img(Size(180,122),CV_8UC3,color[0]);
...
//    cubes.push_back(Cube(points, Vec4f(53,36,155,255),Vec4f(113,86,255,255),Vec4f(1930,1660,2550,2550),Vec4f(0,0,0,255), TX_HALF_RL_HALF_RR, 1.33, quad, sphereTexture2, normalMapCube));
    ...
    vector<Vec3d> dirBuf{Vec3d(0,0,0)};
    vector<Vec3d> dirBuf2{Vec3d(0,0,0)};
    for(int i = 0; i < 36; i ++){
        dirBuf.push_back(Vec3d(-10.0/36,0,0));
        dirBuf2.push_back(Vec3d(0, 10.0/36, 0));
    }
    for(int i = 0; i < 36; i ++){
        dirBuf.push_back(Vec3d(0,-10.0/36,0));
        dirBuf2.push_back(Vec3d(-10.0/36, 0, 0));
    }
    for(int i = 0; i < 36; i ++){
        dirBuf.push_back(Vec3d(10.0/36,0,0));
        dirBuf2.push_back(Vec3d(0, -10.0/36, 0));
    ...
    for(int cntBuf = 0; cntBuf < dirBuf.size(); cntBuf ++){
        ...
        rotater.rotateTowards(dirBuf2[cntBuf], 0);
        colorDeter.spheres = rotater.spheres;
        for(int m = 0; m < M; m ++)
            for(int n = 0; n < N; n ++)
            {
