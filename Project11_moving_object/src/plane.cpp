...
vector<Vec4f> Planes::TexColorDeter(Vec3d ph = Vec3d(0,0,0)){
    ...
    if(planeType == HOMO){
        ...
        double x = (ph - pc).dot(n0)/s0;
        double y = (ph - pc).dot(n1)/s1;
        int xI = min(max(int(round((x-floor(x))*textureMap.cols)),0),textureMap.cols-1);
        int yI = min(max(int(round((y-floor(y))*textureMap.rows)),0),textureMap.rows-1);
//        cout << "xI is " << xI << " yI is " << yI << endl;
        ...
        for(int i = 0; i < 3; i ++){
            ...
            texColors[2][i] = int(min(texColors[1][i]/50.0, 5.1));
        }
        texColors[0][3] = 255;
        texColors[2][3] = 5.1;
        return texColors;
