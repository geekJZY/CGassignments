...
//    cubes.push_back(Cube(points, Vec4f(53,36,155,255),Vec4f(113,86,255,255),Vec4f(1930,1660,2550,2550),Vec4f(0,0,0,255), TX_HALF_RL_HALF_RR, 1.33, quad, sphereTexture2, normalMapCube));
    ...
    vector<double> eyeLightGeneFocusBuf{4.4,4.4,4.4,4.4,4.4,4.4,4.4,4.4};
    for(int i = 0; i < 12; i ++){
        eyeLightGeneFocusBuf.push_back(eyeLightGeneFocusBuf[i+7]+0.4/12);
    }
    for(int i = 1; i < 8; i ++){
        eyeLightGeneFocusBuf.push_back(4.8);
    }
    for(int i = 0; i < 12; i ++){
        eyeLightGeneFocusBuf.push_back(eyeLightGeneFocusBuf[i+27]-0.4/12);
    }
    for(int i = 1; i < 8; i ++){
        eyeLightGeneFocusBuf.push_back(4.4);
    }
    ...
    for(int m = 0; m < M; m ++)
        for(int n = 0; n < N; n ++)
        {
            ...
            colorSum[0] = int(colorSum[0]/(xSample*ySample)/eyeLights.first.size());
            colorSum[1] = int(colorSum[1]/(xSample*ySample)/eyeLights.first.size());
            colorSum[2] = int(colorSum[2]/(xSample*ySample)/eyeLights.first.size());
